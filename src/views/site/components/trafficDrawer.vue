  ...
  <el-drawer
    ...
    <div class="traffic-out">
      ...
      >
        ...
        <el-form-item>
          <bx-button class="searchBtn" @click="searchData" label="搜索"></bx-button>
        </el-form-item>
      </el-form>
      ...
      >
        ...
        <el-table-column prop="traffic" label="流量" align="center">
          <template slot-scope="scope">
            <span>{{(scope.row.traffic / 1024).toFixed(1)}} Kb</span>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    drawLine() {
      ...
      let _max = (Number((this.maxY / 100).toFixed(0)) + 1) * 100
      ...
      this.$nextTick(() => {
        getNginxNodeTraffic(
          ...
          if (_nginxTraffic.length > 9) {
            this.nginxTraffic = (Number(_nginxTraffic) /(1024*1024*1024)).toFixed(1) + ' Gb'
          } else if (_nginxTraffic.length > 6 && _nginxTraffic.length <= 9) {
            this.nginxTraffic = (Number(_nginxTraffic) /(1024*1024)).toFixed(1) + ' Mb'
          } else {
            this.nginxTraffic = (Number(_nginxTraffic) /1024).toFixed(1) + ' Kb'
          }
          this.nginxTrafficVos = res.data.nginxTrafficVos
          this.maxY = (res.data.nginxTimeTrafficMax / 1024) > 1 ? Number((res.data.nginxTimeTrafficMax / 1024).toFixed(0)) : 200
          ...
          this.listY = res.data.timeVos.map(item => {
            if (item.traffic == 0) {
              ...
              return (item.traffic / 1024).toFixed(1)
            } else {
              return 0.1
            }
          })
