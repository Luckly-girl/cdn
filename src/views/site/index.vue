  ...
  <div class="site">
    <el-form ref="searchForm" :inline="true" :model="searchMap" @submit.native.prevent>
      <el-form-item v-if="btnAuthority($route.path, 0)">
        <bx-button class="searchBtn" type="text" @click="handlerAdd(false)" label="添加网站"></bx-button>
      </el-form-item>
    ...
    >
      <el-table-column label="状态" align="center" min-width="60">
        <template slot-scope="scope">
          <img src="../../assets/images/blue.png" alt v-if="scope.row.serverStatus===1" />
          <el-popover
            ...
            <el-table :data="scope.row.servers" :show-header="false">
              <el-table-column width="300" label>
                <template slot-scope="sco">
                  <span>从{{sco.row.nodeIp}} 到 {{sco.row.serverIp}} 无法连接</span>
                </template>
              </el-table-column>
            </el-table>
      ...
      >
        <template slot-scope="scope">
          <div
            class="pointer"
            @click="handleApplyCertificate(scope.row.applyStatus,scope.row.id, scope.row.productId, scope.row.pemType)"
          >
            <div v-if="scope.row.applyStatus == '2'">
              <span style="color:#15b64e; padding-bottom:3px;">申请成功（{{scope.row.pemType == 0 ? '自动' : '手动'}}）</span>
            </div>
            <div v-else-if="scope.row.applyStatus == '4'">
              <span style="color:#fa4c4c; border-bottom:1px solid #fa4c4c; padding-bottom:3px;">申请失败</span>
            </div>
            <div v-else-if="scope.row.applyStatus == '3'">
              <span style="color:#ffae00; border-bottom:1px solid #ffae00; padding-bottom:3px;">申请中</span>
            </div>
            <div v-else-if="scope.row.applyStatus == '1'">
              <span style="color:#ffae00; border-bottom:1px solid #ffae00; padding-bottom:3px;">正在申请</span>
            </div>
            <div v-else>
              <span style="color:#999999; border-bottom:1px solid #999999; padding-bottom:3px;">未申请</span>
            </div>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        :filter-multiple="false"
        :filter-method="filterTag"
        :filtered-value="searchMap.nginxState=='' ? [] : [searchMap.nginxState]"
      >
        <template slot-scope="scope">
          ...
          >
            <div slot="reference">
              <span @click="deployNginxFile(scope.row.id)" style="color:#999999;border-bottom: 1px solid #999;">预部署</span>
            </div>
          </el-popover>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          ...
          <div v-else>
            <span style="color:#4d71ee; border-bottom:1px solid #4d71ee;cursor: pointer;" v-if="scope.row.dnsKidnap>0" @click="getSiteDnsKidnapData(scope.row.id)">{{scope.row.dnsKidnap}}</span>
      ...
      <el-table-column label align="center" min-width="60" v-if="btnAuthority($route.path, 0)">
        <template slot-scope="scope">
          <div
      ...
      <el-table-column label="日志统计" align="center" min-width="80">
        <template slot-scope="scope">
          <i
            class="iconfont iconbutton_trend act-img-btn"
            @click="logStatisticsClick(scope.row.id, scope.row.realmName, scope.row.isFilebeat, scope.row.productId)"
          ></i>
        </template>
      </el-table-column>
      <el-table-column
    ...
    <!-- DNS劫持列表 -->
    <el-dialog :visible.sync="dialogVisibleDnsKidnapData" width="626px" :close-on-click-modal="false">
    ...
    <div class="add">
      ...
      >
        <el-menu :default-active="activeIndex1" mode="horizontal">
          <el-menu-item index="1">{{ reuse ? '复用cname' : '增加网站' }}</el-menu-item>
        </el-menu>
        ...
        <div class="add-site">
          ...
          >
            ...
            <el-form-item label="协议类型" prop="type">
              <el-select v-model="addSiteForm.type" placeholder="请选择协议类型">
                <el-option
                  v-for="item of agreementList"
                  :key="item.code"
                  :value="item.code"
            ...
            <el-row>
              <el-col :span="15">
                <el-form-item label="端口号" prop="portNum">
                  <el-input v-model="addSiteForm.portNum" placeholder="请填写端口号"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="9">
                ...
                <el-row>
                  <el-col :span="11">
                    <div class="iptype-out">
                      <el-dropdown trigger="click" @command="handleCommand" class="el-dropdown-ip">
                        ...
                        <el-dropdown-menu slot="dropdown">
                          <el-dropdown-item
                            ...
                            :key="siteTypeIndex"
                            :command="{index: index, oldVal: item.ipType, newVal: siteTypeIndex}"
                  ...
                  <el-col :span="7">
                    <el-form-item label="IP类型" label-width="58px">
                      <el-select class="input2 ip-select" filterable v-model="item.type">
                        <el-option
                  ...
                  <el-col :span="6">
                    <el-form-item class="add-ips-btn" label-width="40px">
                      <bx-button
                        ...
                        label="添加"
                      ></bx-button>
                      <el-button v-else class="yuming cancel" @click="delIP(index)">删除</el-button>
                      <el-switch
                        class="switchStyle"
            ...
            <div v-if="addSiteForm.type==2">
              <el-form-item label="源IP私钥文本域" label-width="105px" prop="originPrivkey">
                <el-input
                  type="textarea"
                  :autosize="{ minRows: 2, maxRows: 4}"
                  placeholder="请输入内容"
              ...
              <el-form-item label="源IP证书文本域" label-width="105px" prop="originFullchain">
                <el-input
                  type="textarea"
                  :autosize="{ minRows: 2, maxRows: 4 }"
        ...
        <div slot="footer" class="dialog-footer">
          <el-button class="reset" @click="resetForm('addSiteForm')">重置</el-button>
          <bx-button
            class="confirm"
            :loading="addLoading"
            type="primary"
    ...
    <div class="edit">
      ...
      >
        ...
        >
          ...
          <el-menu-item index="2" v-if="btnAuthority($route.path, 3)">站点分配</el-menu-item>
        </el-menu>
              ...
              >
                <div class="normal">
                    ...
                    <div class="inner">
                      <el-row>
                        <el-col :span="14">
                          ...
                          >
                            <el-input v-model="base.realmName[index]"></el-input>
                          </el-form-item>
                        </el-col>
                        <el-col :span="4">
                          <el-form-item label-width="10px">
                            <div class="add-del-ico">
                              <i class="el-icon-circle-plus-outline" v-if="index==0" @click="addHost('base')"></i>
                              <i class="el-icon-delete" v-if="base.realmName.length>1" @click="delHost(index,'base')"></i>
                            </div>
                          </el-form-item>
                        </el-col>
                        <el-col :span="4">
                  ...
                  <el-row>
                    <el-col :span="15">
                      <el-form-item label="端口号" prop="portNum">
                        <el-input v-model="base.portNum" placeholder="请填写端口号"></el-input>
                      </el-form-item>
                    </el-col>
                    <el-col :span="9">
                      <span class="font-tail">不填写会使用默认端口号</span>
                  ...
                  <el-row>
                    <el-col :span="6">
                      <el-form-item label="重定向" prop="redirect" class="mb-20">
                        <el-switch
                    ...
                    <el-col :span="18">
                      <span class="font-tail">注意：更改重定向则cname相同站点重定向会同步更改</span>
                    </el-col>
                  </el-row>
                      ...
                      <el-row>
                        ...
                        <el-col :span="6">
                          <el-form-item label-width="0px">
                            <div class="add-del-ico">
                              <i class="el-icon-circle-plus-outline" v-if="index==0" @click="addIPBase"></i>
                              <i class="el-icon-delete" v-if="base.ipTypes.length>1" @click="delIPBase(index)"></i>
                            </div>
                            <el-switch
                              class="switchStyle"
                              v-model="item.ableStatus"
                  ...
                  <div v-for="(item,index) in base.localNames" :key="index">
                    <div class="inner">
                      <el-row>
                        ...
                        <el-col :span="5">
                          <el-form-item label-width="20px">
                            <div class="add-del-ico">
                              <i class="el-icon-circle-plus-outline" v-if="index==0" @click="addLocalBase"></i>
                              <i class="el-icon-delete" v-if="base.localNames.length>1" @click="delLocalBase(index)"></i>
                            </div>
                          </el-form-item>
                        </el-col>
                      </el-row>
                      <el-form-item
                        ...
                        :rules="rules.lips"
                      >
                        <el-input v-model="item.lips" placeholder="多个IP请用,隔开"></el-input>
                      </el-form-item>
                      <el-form-item
                        label="反向代理路径"
                        label-width="95px"
                ...
                <div class="normal" v-if="base.type==2 && !redirect">
                  ...
                  <el-row>
                    <el-col :span="12">
                      <el-form-item label="源IP证书文本域" prop="originFullchain">
                        <el-input
                          type="textarea"
                          :autosize="{ minRows: 2, maxRows: 4 }"
          ...
          <el-tab-pane label="安全" name="safe" v-if="!redirect">
            ...
            >
              <div class="normals">
                <el-row>
                  <el-col :span="16">
                    <el-form-item label="白名单" prop="whiteLists">
                      <el-input v-model="safe.whiteLists" placeholder="请输入白名单"></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                ...
                <el-row>
                  <el-col :span="16">
                    <el-form-item label="黑名单" prop="blackLists">
                      <el-input v-model="safe.blackLists" placeholder="请输入黑名单"></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <span class="font-tail">多个IP请用 “,” 隔开</span>
                ...
                <el-form-item label="日志分析黑名单" prop="logAnalysis" label-width="120px">
                  <el-switch
                    :active-color="theme.backgroundColor"
                    :inactive-value="0"
                    :active-value="1"
                ...
                <div v-show="safe.logAnalysis" class="_safe-log">
                  <el-row>
                    <el-col :span="15">
                      <el-form-item label="封禁时间" prop="banTime" label-width="80px">
                  ...
                  <div class="ban-tip">封禁时间请填写正整数，不填写默认封禁2小时</div>

                  <el-row>
                    <el-col :span="15">
                      <el-form-item label="封禁规则" prop="banRole" label-width="80px">
                  ...
                  <div class="ban-tip">
                    ...
                    <br />例:填写10 则一分钟内同一ip访问超过10次则会加入日志黑名单
                  </div>
                </div>
                ...
                <div>
                  ...
                  <el-form-item label="CC类型" prop="ccType" class="_safe-cc" v-show="safe.ccSwitch">
                ...
                <el-row>
                  ...
                  <el-col :span="17">
                    <span
                      class="font-tail antiProxyIp-tail"
                    >注意：开启该功能之后，部分手机浏览器加速功能会导致站点不可访问，关闭手机加速功能后可访问</span>
                  </el-col>
                </el-row>

                <div>
                  <el-form-item label="waf防护" prop="safeSwitch" class="_safe-cc">
                    <el-switch
            ...
            >
              <div class="normals">
                <el-form-item label="手机号" prop="phoneNumber">
                  <el-input v-model="contactMsg.phoneNumber" placeholder="请输入手机号" maxlength="11"></el-input>
                </el-form-item>
                <el-form-item label="QQ" prop="qqNumber">
                  <el-input v-model="contactMsg.qqNumber" placeholder="请输入qq号"></el-input>
                </el-form-item>
                <el-form-item label="电子邮箱" prop="email">
                  <el-input v-model="contactMsg.email" placeholder="请输入电子邮箱"></el-input>
                </el-form-item>
                <el-form-item label="Skype" prop="skype">
                  <el-input v-model="contactMsg.skype" placeholder="请输入Skype"></el-input>
                </el-form-item>
              </div>
            </el-form>
          </el-tab-pane>
          ...
          <el-tab-pane label="更多" name="more" v-if="!redirect">
            ...
            >
              ...
              <div class="reserve inner" v-show="more.gzip">
                <el-form-item label="gzip支持的压缩文本类型" label-width="180px" class="check">
              ...
              <div class="reserve inner" v-show="more.cache">
                <el-form-item label="需要缓存的文件类型" label-width="200px" class="check">
                ...
                <el-form-item
                  ...
                  prop="cacheTime"
                >
                  <el-input v-model="more.cacheTime" style="width: 200px;"></el-input>
                ...
                <div class="ban-tip other-tip">缓存时间请填写正整数,不填写默认缓存30天</div>
              </div>
              ...
              <div class="reserve inner20" v-show="more.limits">
                <el-row>
                  <el-col :span="18">
                    <el-form-item label="单个IP最大连接数限制" label-width="150px" prop="ipNum">
                      <el-input placeholder v-model="more.ipNum"></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="6" class="font-limit-bg">
                    <span class="font-limit unCopy">个/s (不填写默认10个/s)</span>
                  </el-col>
                </el-row>

                <el-row>
                  <el-col :span="18">
                    <el-form-item label="网站的最大连接数限制" label-width="150px" prop="serverNum">
                      <el-input placeholder v-model="more.serverNum"></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="6" class="font-limit-bg">
                    <span class="font-limit unCopy">个/s (不填写默认500个/s)</span>
                  </el-col>
                </el-row>

                <el-row>
                  <el-col :span="18" class="font-limit-bg">
                    <el-form-item label="网站的最大下载速率" label-width="150px" prop="downloadRate">
                      <el-input placeholder v-model="more.downloadRate" maxlength="10"></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="6" class="font-limit-bg">
                    <span class="font-limit unCopy">k/s (不填写默认200k/s，最大2,000,000,000k/s)</span>
                  </el-col>
                </el-row>
              </div>
              <div class="normal">
                ...
                <el-form-item label="反向代理支持跨域" prop="reverseProxyCrossDomain">
                  <el-switch
                    :active-color="theme.backgroundColor"
        ...
        <div slot="footer" class="dialog-footer" v-if="activeIndex2 == '1'">
          <el-button class="reset" @click="resetForm(activeTabName, 'edit')">取消</el-button>
          <bx-button class="confirm" type="primary" @click="submitEdit" label="保存">保存</bx-button>
        </div>
      ...
      <div class="log-statistics" v-if="logShow">
        <div class="header">
          ...
          <div class="header-right">
            <i class="el-dialog__close el-icon el-icon-close" @click="goBack"></i>
          </div>
        </div>
  ...
  data() {
    ...
    const lipsRule = (rule, value, callback) => {
      ...
      if (value == "" || value == undefined || value == null) {
        if (this.base.localNames[_thisIndex].localName == "" && this.base.localNames[_thisIndex].localUrl == "") {
          callback();
        } else {
    ...
    const localNameRule = (rule, value, callback) => {
      ...
      let _thisIndex = Number(rule.field.split(".")[1]);
      if (value == "" || value == undefined || value == null) {
        if (this.base.localNames[_thisIndex].lips == "" && this.base.localNames[_thisIndex].localUrl == "") {
          callback();
        } else {
    ...
    return {
      ...
      moreRules: {
        ...
        downloadRate: [{ required: true, validator: downloadRateRule, trigger: "blur" }]
      },
      sslRules: {
        privkey: [
          { required: true, message: "私钥文本域内容不能为空", trigger: "blur" }
        ],
        fullchain: [
          { required: true, message: "证书文本域内容不能为空", trigger: "blur" }
        ]
      },
      msgRules: {
          ...
          if (this.base.localNames.length <= 0) {
            ...
            if (
              this.base.localNames.some(item => {
                return (
                  (item.localName === "" && item.lips === "" && item.localUrl !== '') ||
    ...
    editSiteSafe() {
      this.$refs["safe"].validate(valid => {
        if (valid) {
          ...
          this.safe.safeValue = this.safe.safeValue.filter(item => item != "");
...
.site .iptype-choose .el-form-item__content {
...
.site .edit .el-form-item__content {
  min-height: 32px;
  display: flex;
  align-items: center;
}
