  ...
  <div class="perm">
    <el-form ref="searchForm" :inline="true" :model="searchMap" @submit.native.prevent>
      <el-form-item v-if="btnAuthority($route.path, 0)">
        <bx-button class="searchBtn" type="text" @click="handlerAddNode" label="开通权限"></bx-button>
      </el-form-item>

      <el-form-item label="权限名称" prop="name">
        <i class="icon-search pointer" @click="fetchData()"></i>
        <input class="input" @keyup.enter="fetchData()" v-model="searchMap.name" placeholder="请输入权限名称" />
      </el-form-item>
    </el-form>

    <el-table class="mt-18"
      :data="list"
    ...
    >
      ...
      <el-table-column prop="type" label="权限分类" align="center"></el-table-column>
      <el-table-column prop="page" label="权限路径" align="center"></el-table-column>
      <el-table-column prop="code" label="权限Code" align="center"></el-table-column>
      <el-table-column prop="zindex" label="优先级" align="center"></el-table-column>

      <el-table-column label="操作" align="center" width="180" v-if="btnAuthority($route.path, 0) || btnAuthority($route.path, 1) || btnAuthority($route.path, 2)">
        <template slot-scope="scope">
          <i v-if="(scope.row.pid=='0'||!scope.row.pid) && btnAuthority($route.path, 0)" class="iconfont iconbutton_increase act-img-btn" @click="handleEdit(scope.row,'child')"></i>
          <i class="iconfont iconbutton_edit act-img-btn" :style="{'padding-left': scope.row.pid=='0'||!scope.row.pid ? 0 : '48px'}" @click="handleEdit(scope.row)" v-if="btnAuthority($route.path, 1)"></i>
          <i class="iconfont iconbutton_delete act-img-btn" @click="handlerDelete(scope.row.id)" v-if="btnAuthority($route.path, 2)"></i>
        </template>
      </el-table-column>
    </el-table>

    <el-dialog :visible.sync="dialogVisible" width="626px" @closed="resetForm" :close-on-click-modal="false">
      ...
      <div class="add-permission">
              ...
              <el-col :span="12" class="pl-18">
                <el-form-item label="图标" prop="icon">
                  <el-popover
                    ...
                    trigger="click">
                    <el-row>
                      <el-col :span="6" v-for="(item,index) in iconfontList" :key="index" class="icon-col">
                        <el-button type="img" size="auto" @click.native="userFrom.icon = item; iconselect = false">
        ...
        <div class="dialog-footer">
          ...
          <bx-button class="confirm" type="primary" @click="setPerm()" label="确认"></bx-button>
        </div>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      findPerms(this.searchMap.name).then(res => {
      ...
      this.$refs['userFrom'].validate(valid => {
        if (valid) {
          setPerm({
            ...
            this.$store.dispatch('GetUserPerms').then(res => { // 拉取菜单
              this.$store.dispatch('GenerateRoutes').then((resss) => { // 根据roles权限生成可访问的路由表
