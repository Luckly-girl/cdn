<template>
  <div class="user">
    <el-form ref="searchForm" :inline="true" :model="searchMap" @keyup.enter.native="fetchData()">
      <el-form-item v-if="btnAuthority($route.path, 0)">
        <bx-button class="searchBtn" @click="handlerAdd" label="开通用户"></bx-button>
      </el-form-item>
      ...
      <el-form-item>
        <el-button class="searchBtn btn-reset" @click="reset1()">重置</el-button>
      </el-form-item>
      <el-form-item>
        <bx-button class="searchBtn" @click="fetchData()" label="查询"></bx-button>
      </el-form-item>
    </el-form>
    ...
    <el-table
      ...
      <el-table-column prop="roleNames" label="角色名称" align="center"></el-table-column>
      ...
      <el-table-column label="操作" align="center" width="130" v-if="btnAuthority($route.path, 1) || btnAuthority($route.path, 2)">
        <template slot-scope="scope">
          <i class="iconfont iconbutton_edit act-img-btn" @click="handlerEdit(scope.row)" v-if="btnAuthority($route.path, 1)"></i>
          <i class="iconfont iconbutton_delete act-img-btn" @click="handlerDelete(scope.row)" v-if="btnAuthority($route.path, 2)"></i>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="dialogVisible" width="626px" @close="init" :close-on-click-modal="false">
      ...
      <add-user ref="myChild" @showbox="toshow" :parentmsg="pupotName" :userID="id"></add-user>

      <div slot="footer" class="dialog-footer">
        <el-button class="reset" @click="resetForm()">重置</el-button>
        <bx-button class="confirm" type="primary" @click="updateDate()" label="确认"></bx-button>
      </div>
    </el-dialog>
  </div>
</template>
