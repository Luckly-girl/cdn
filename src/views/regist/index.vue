<template>
  <div class="login" id="loginSubmitBtn">
        ...
        <el-form :model="userInfo" :rules="registRules" ref="userInfo">
          ...
          <el-form-item prop="mcode" class="input-bg phoneNumberItem">
            ...
            >
              ...
              <span v-show="!codeShow" class="count"
                >重新获取（{{ count }} s）</span
              >
            </el-button>
          </el-form-item>
          ...
          <el-form-item class="btn loginBtn">
            <el-button class="submitBtn dis center" @click="handlerSubmitBtn"
              >注 册</el-button
            >
          </el-form-item>
          <el-form-item>
            <div class="registerBtn">
              已有账号？<span @click="loginBtn">立即登录</span>
            </div>
          </el-form-item>
        </el-form>
      </div>
    ...
    <div class="login-bottom-bg dis center">
      ...
      鄂公网安备 42018502004475号<br />增值电信业务经营许可证号: 鄂B2-20190165
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    //     48 -
    //     parseInt(
    //       (new Date().getTime() - localStorage.getItem("codeTime")) / 1000
    //     );
    //   this.codeShow = false;
  ...
  methods: {
    ...
    handlerSubmitBtn() {
      this.$refs["userInfo"].validate((valid) => {
        if (valid) {
          checkMcode(this.userInfo.phoneNumber, this.userInfo.mcode).then(
            (res) => {
              let _userInfo = {
...
#loginSubmitBtn .el-form-item__content {
  display: flex;
  align-items: center;
  height: 100%;
}
