  ...
  <div class="navbar" :style="{'backgroundColor': theme.backgroundColor}">
    <div class="navbar-bg">
      <el-scrollbar class="default-scrollbar" wrap-class="default-scrollbar__wrap" view-class="default-scrollbar__view">
        ...
        >
          <el-submenu :index="'tab'+index" v-for="(parent, index) in parentsList" :key="index">
            <template slot="title">
              <item :addclass="'font-nav'" v-if="parent" :icon="parent.icon" disIcon="true" :title="parent.name"/>
              <!--              <img class="icon-nav" src="@/assets/images/cdn/icon_accounts.png" alt="">-->
              <!--              <span class="font-nav" :class="{ navColor: index==='3' ? true : false }">{{parent.name}}</span>-->
            </template>
            <el-menu-item-group>
              <el-menu-item :index="`/${child.page}`" v-for="(child,i) in parent.child" :key="i">
                <item v-if="child" :icon="child.icon" disIcon="true" :title="child.name"/>
      ...
      <div class="pro-version">
        ...
        >
          <div class="nav-update-list">
            <p><span>总节点：</span><span>{{versionInfo.versionCount.nodeCount}}个</span></p>
            <p><span>失败节点：</span><span>{{versionInfo.versionCount.nodeErrorCount}}个</span></p>
            <p><span>成功节点：</span><span>{{versionInfo.versionCount.nodeSuccessCount}}个</span></p>
            <p><span>更新中节点：</span><span>{{versionInfo.versionCount.nodeUpdatingCount}}个</span></p>
          </div>
          <div slot="reference">
            <div class="update-btn updateing">更新中</div>
          </div>
        </el-popover>
        <div class="update-btn" v-if="versionInfo.updateFlag==1&&versionInfo.versionCount.nodeUpdatingCount==0" @click="versionUpdate">更新</div>
