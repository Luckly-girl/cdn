  ...
  >
    <div slot="title" class="dialog-header">
      ...
      <div v-else-if="dialogType=='allList'" class="all-list-title">
        全部通知
        <bx-button class="all-read-btn" @click="addRead" label="全部标记为已读"></bx-button>
      </div>
      ...
      <div>
        ...
        <div class="all-msg-bg" v-else>
          <ul class="all-msg-list">
            <li v-for="(item, index) in msgList" :key="index" @click="getNoticeInfo(item.id)">
              <div class="msg-content-bg">
                <div class="icon" :class="[item.sureRead==1?'already-read':'']"></div>
                <div class="msg-content">
                  <span :class="[item.sureRead==1?'fc-999':'']">{{item.noticeTitle}}</span>
    ...
    <div v-else class="msg-detail-main">
      ...
      <div class="msg-detail-content" v-html="$options.filters.msgContentFilter(msgDetail.content)"></div>
